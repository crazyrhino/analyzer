...
import com.tencent.research.nlp.WP;

public class AresTCTokenizer extends Tokenizer {
	private static final Logger logger = Logger
			.getLogger(AresTCTokenizer.class);

	public native static int init(String conf);

	public native static String SegString(String name);

	private SWIGTYPE_p_void handle;
	...
	public AresTCTokenizer(Reader in, SWIGTYPE_p_void handle) {
		super(in);
		init();
		this.handle = handle;
	}
	...
	public void seg() {
		try {
			...
			// 空白字符处理以及大写转换
			resultArray = this.getSegString(sentence);
		} catch (Exception e) {
			logger.error("seg error ", e);
		}
	}

	private String[] getSegString(String str) throws IOException {
		// TCWordSeg.TCSegment(handle, str.trim().toUpperCase().getBytes());
		TCWordSeg.TCSegment(handle, to_cstr_bytes(str.trim().toUpperCase()));
		int rescount = TCWordSeg.TCGetResultCnt(handle);
		String[] result = new String[rescount];

		for (int i = 0; i < rescount; i++) {
			try {
				WP wp = TCWordSeg.TCGetAt(handle, i);
				StringBuilder sb = new StringBuilder();
				sb.append(new String(wp.getWord(), "GBK"));
				sb.append("/" + wp.getPos());
	...
	private boolean next() throws IOException {
		...
		if (index == 0) {
			...
			repositionOffset(original.toString());
		}
			...
			if (isStartTag) {
				for (int j = 0; j < tag.endTags.length; j++) {
					...
					if (originalIndex > -1) {// 如果包含HTML结束标签
						...
						int step = this.getSegString(filted).length;
	...
	public static byte[] to_cstr_bytes(String str) throws IOException {
		String s = str + '\0';
